defmodule ImageHub.MultimediaTest do
  ...
  describe "videos" do
    ...
    def user_fixture(attrs \\ %{email: "some_email@test.com", name: "Test", password: "somepass"}) do
      {:ok, user} = ImageHub.Accounts.create_user(attrs)
    ...
    def video_fixture(attrs \\ %{}) do
      ...
      {:ok, video} = user |> Multimedia.create_video(attrs)
    ...
    test "get_video!/1 returns the video with given id" do
      video = video_fixture()
      assert Multimedia.get_user_video!(video.user, video.id) == video
    end
