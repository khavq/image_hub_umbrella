defmodule ImageHubWeb.Channels.VideoChannelTest do
  use ImageHubWeb.ChannelCase
  import ImageHubWeb.TestHelpers

  setup do
    user = insert_user(name: "Gary")
    video = insert_video(user, title: "Testing")
    token = Phoenix.Token.sign(@endpoint, "user socket", user.id)
    {:ok, socket} = connect(ImageHubWeb.UserSocket, %{"token" => token})
  ...
  test "join replies with video annotations", %{socket: socket, video: vid, user: user} do
    for body <- ~w(one two) do
      ImageHub.Multimedia.annotation_video(user, vid.id, %{body: body, at: 0})
    end
  ...
  test "inserting new annotation", %{socket: socket, video: vid} do
    ...
    ref = push socket, "new_annotation", %{body: "some body", at: 0}
    assert_reply ref, :ok, %{}
    assert_broadcast "new_annotation", %{}
  end

  test "broadcast annotation with info_sys wolfram", %{socket: socket, video: vid} do
    insert_user(email: "wolfram@imh.com", password: "supersecret")
